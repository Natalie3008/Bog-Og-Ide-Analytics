...
class testAnalytics {
	...
	public void testProductsAnalytics() {
		// Arrange
		ArrayList<Product> getProductsAnalytics = new ArrayList<Product>();

		// Act
		try {
			getProductsAnalytics.addAll(saleDB.getProductsAnalytics("Slow", "Book", 2019, 11, 27, 2));
		} catch (SQLException e) {
			e.printStackTrace();
		}

		//Assert
		assertEquals("Return Bestsellers", getProductsAnalytics.size(), 2019);
	}
	...
	public void testIncorrectParameterChoice() {
		...
		// Act
		try {
			foundproduct.addAll(saleDB.getProductsAnalytics("No Sale", "Book", 2019, 11, 27, 2));
		} catch (SQLException e) {
			e.printStackTrace();
		}

		//Assert
		assertEquals("Return empty list", foundproduct.get(0));
	}
	...
	public void testSalesAnalytics() {
		...
		//Assert
		assertEquals("Return Bestsellers", foundproducts.size());
		}
